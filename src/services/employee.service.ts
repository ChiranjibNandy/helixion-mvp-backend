...
export const enrollInProgramService = async (
  ...
  // convert once  reused across checkExisting, countForProgram, and createEnrollment calls
  ...
  const enrollment = await createEnrollmentRepo({
    ...
    stayType,
    // fee locked at enrollment time to prevent disputes if program price changes
    feeAmount: resolveEnrollmentFee(program, stayType),
    currency:  CURRENCY.INR,
    // program snapshot frozen at enrollment time for historical accuracy
    programSnapshot: {
      title:               program.title,
